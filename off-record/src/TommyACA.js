...
function TommyACA({BGpic,tapImage,PFLEvents, adminKevPickswID, BellatorInfo, state, user,mewtwo,adminKevPicks2, ufcCard, stallUfcCard,locationCity,location, isOwnerAndEventMatch, setjustSubmitted, onLogout, weRlive}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://off-therecordpicks.onrender.com/aca-fights');
...
console.log( ufcCard)
  ...
  useEffect(() => {
    async function submitPFLEvent() {
      try {
        ...
        const postResponse = await axios.post('https://off-therecordpicks.onrender.com/submit-aca-event', dataToSend, {
          headers: {
        ...
        if (postResponse.status === 201) {
          // Handle success
          console.log('ACA event submitted successfully:', postResponse.data);
  ...
  const patchEvent = () => {
    ...
    fetch(`https://off-therecordpicks.onrender.com/aca-events/${oldEvent.id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then(async (response) => {
        ...
        if (response.ok) {
          ...
          console.log('ACA Card patched successfully:', responseData);
          // Perform any further actions here
        }
 ...
 : */}
    ...
    <div className="tommy">
      ...
      <div className="bayLoc"  > 
          ...
          <center style={{backgroundColor:'whitesmoke'}}><h1 style={{
                              height: '50px',
